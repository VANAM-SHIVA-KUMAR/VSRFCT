      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {branches.map((branch, index) => (
            <Link href={branch.href} key={index} className="block">
              <div
                className={cn(
              ...
              >
                <Card className="text-center hover:shadow-lg transition-all duration-300 h-full bg-White-100 overflow-hidden">
                  <CardHeader className="p-6">
                    <div className={cn("mx-auto bg-primary/10 rounded-full p-4 w-20 h-20 flex items-center justify-center mb-4 transition-transform duration-300 group-hover:scale-110")}>
                      {branch.icon}
                    </div>
